              ...
              <Row>
                <Col span="24">
                  ...
                  <Col span="8" v-if="product.deliveryExpressEnabled">
                      <FormItem label="运费模板" class="ivu-form-item-required">
  ...
  methods: {
    ...
    addok(name) {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          if (!_this.product.deliveryExpressEnabled&&!_this.product.deliveryCityEnabled&&!_this.product.deliveryPickEnabled) {
            _this.$Message.error("请选择至少一种配送方式");
            return;
          }
          if (_this.product.deliveryExpressEnabled&&!_this.selectedFreight) {
            _this.$Message.error("运费模板必选");
            return;
          }
          if (_this.product.deliveryCityEnabled&&!_this.selectedDelivery) {
